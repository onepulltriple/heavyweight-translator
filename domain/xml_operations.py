...
def split_string_into_list_of_tagged_and_untagged_elements(input_str):
    input_str_with_xml_wrapper = (
        f"<?xml version=\"1.0\" encoding=\"utf-8\" ?><paragraph>{input_str}</paragraph>"
    )
    ...
    try:
        ...
        try:
            input_str_with_xml_wrapper = (
                re.sub(r"<run (\w*)=\"(\d*)\">(.*)</paragraph>", 
                       r'<run \1="\2">\3</run></paragraph>', 
                       input_str_with_xml_wrapper,
                )
            )
            root = ET.fromstring(input_str_with_xml_wrapper)
            ...
            xml_debug_file_path = (
                FP.file_path_dictionary["start_of_xml_debug_file_path"]
                + error_time_stamp
                + FP.file_path_dictionary["end_of_xml_debug_file_path"]
            )
